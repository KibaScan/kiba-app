 ...
 * Raw bookmark rows without product or score data. Use `fetchBookmarkCards` instead if you need product info and live scores.
 */
export async function getBookmarksForPet(petId: string): Promise<Bookmark[]> {
  ...
  try {
    ...
    if (error) {
      console.warn('[getBookmarksForPet]', error.message);
      return [];
    }
    return (data ?? []) as Bookmark[];
  } catch (err) {
    console.warn('[getBookmarksForPet]', err);
    return [];
  }
}
...
export async function fetchBookmarkCards(pet: Pet): Promise<BookmarkCardData[]> {
  ...
  try {
    ...
    if (error) {
      console.warn('[fetchBookmarkCards]', error.message);
      return [];
    }
    ...
    console.warn('[fetchBookmarkCards]', err);
    return [];
  }
}
